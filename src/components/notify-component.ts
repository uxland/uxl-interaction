...
const renderCloseButton = (props: NotifyComponent) =>
  props.options.showCloseButton
    ? html`
        <paper-icon-button @click="${props.closeToast}" id="action-btn" class="close-btn ${props.options.classifiers}" icon="icons:close">
        </paper-icon-button>
      `
    : html``;
...
const renderContent = (props: NotifyComponent) => (props.options.message ? renderCloseButton(props) : renderCustomContent(props));
