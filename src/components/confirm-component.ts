...
const renderCloseButton = (props: ConfirmComponent) =>
  props.options.showCloseButton
    ? html`
        <paper-icon-button @click="${props._close}" id="close-btn" icon="icons:close"></paper-icon-button>
      `
    : nothing;
...
const renderContent = (options: ConfirmOptions, props: ConfirmComponent) =>
  options.message ? renderMessage(options) : renderCustomContent(options, props);
...
export class ConfirmComponent extends LitElement {
  ...
  updated(changedProperties) {
    ...
    styles.actionsBackgroundColor &&
      ...
      this.actionsContainer.style.setProperty('background-color', styles.actionsBackgroundColor);
