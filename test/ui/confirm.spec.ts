...
import {ConfirmOptions, doConfirm} from "../../src/confirm";
...
describe('When invoking `confirm` method', () =>{
    ...
    it('should remove confirm-component from document body when closed event was dispatched', async () => {
        const options : ConfirmOptions = {title: 'my title', message: 'hello from dialog'};
        let {stub, mock} = createStub();
        let resolved = false;
    ...
    it('localize text when localize property is set', async () => {
        ...
        const options: ConfirmOptions = {title: 'app.title', message: 'app.message', titleArgs: {titleArg1: 1}, messageArgs: {messageArg1: 1}};
        const options2: ConfirmOptions = {title: 'component.title', message: 'component.message', titleArgs: {titleArg1: 2}, messageArgs: {messageArg1: 2}};
    ...
    it('should import custom component and set model property', async() =>{
        ...
        let customConfirm: any = dialog.querySelector('custom-confirm');
        assert.exists(customConfirm);
        assert.deepEqual(customConfirm.model, {arg: 'arg1'});
    });

    //TODO comprovar mixin close (result : boolean)
    //
});